...
# plots for overview to help decide which section to extract for training
def subplots(dataset, columns, km_start, km_end):

    dataset.plot(x="Tunnel Distance [m]",
                 y=columns, subplots=True,
                 xlim=(km_start,km_end)
                 )  
...
# extract certain sections from the whole dataset and scale it
def create_sections(tunnel,
    ...
    # linear interpolation where class >=4
     
    for df in [val_df, train_df]:
    
        for col in columns:
            
            # Find indices where GI is 3
    ...
    # takes target column and defines it as new df
    ...
    for i in range(6):
        count = (train_target_df['Class'] == i).sum()
        print(f'total occurences of Class{i} in train dataset: {count}')
        
    for i in range(6):
        count = (val_target_df['Class'] == i).sum()
        print(f'total occurences of Class{i} in validation dataset: {count}')
    
    # defines and fits scaler
...
# creates sequences for LSTM layers
...
def Create_torch_dataset(dataset,
    ...
    # train_df, test_df, val_df = TBMDataset(train_df), TBMDataset(test_df), TBMDataset(val_df)
    ...
    return train_df, test_df1, test_df2, test_df3, val_df
...
if __name__ == "__main__":
    ...
    if tunnel == 'Synth_BBT':
        ...
        df_spec_tor.to_csv('spec. penetration and torque ratio.csv', sep=',', index=False, encoding='utf-8')
...
# =============================================================================
    ...
    train_df, test_df1, test_df2, test_df3, val_df = Create_torch_dataset(
        df,
        seq_size,
        columns,
        target_column,
